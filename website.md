Build a simple single-page web app for my DeepResearch MCP that lets users generate their personal MCP URL without editing any JSON files.
Tech stack: plain HTML + CSS + JavaScript. No frameworks. Must be a single index.html file.
The page must have:
1. A clean hero section explaining what DeepResearch MCP does in 2-3 lines
2. A "Get Started" section with:

Input field for Groq API key (with link to console.groq.com)
Input field for Tavily API key (with link to app.tavily.com)
"Generate My URL" button

3. After clicking the button:

Show the generated URL:

https://deep-research-production-d1d2.up.railway.app/sse?groq_key=INPUT1&tavily_key=INPUT2

A "Copy URL" button next to it
Step by step instructions below showing exactly how to add it to Claude Desktop and Claude.ai

4. A FAQ section answering:

"Are my API keys safe?" — keys never leave the browser, never stored on any server
"How much does it cost?" — free, user pays only their own API usage
"What is Groq / Tavily?" — brief explanation with signup links

the website design is ststed below. make it rersponsive for both mobile as well as laptops, desktops
